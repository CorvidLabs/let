...
/** Keep only panels whose backing agent or project still exists. */
export function retainFleetOpenKeys(
...
export function fleetLiveChangeAnnouncement(
  ...
  current: readonly Pick<FleetAgentActivity, "agent" | "status">[],
): string {
  if (previous.length === 0) {
    return "";
  }
  const previousByAgent = new Map(
    previous.map((agent) => [agent.agent, agent.status]),
  );
  const currentByAgent = new Map(
    current.map((agent) => [agent.agent, agent.status]),
  );
  const changes: string[] = [];
  for (const agent of current) {
    const prior = previousByAgent.get(agent.agent);
    if (!prior) {
      changes.push(`${agent.agent} is now visible.`);
    } else if (prior !== agent.status) {
      changes.push(`${agent.agent} is now ${agent.status}.`);
    }
  }
  for (const agent of previous) {
    if (!currentByAgent.has(agent.agent)) {
      changes.push(`${agent.agent} is no longer visible.`);
    }
  }
  return changes.join(" ");
}
...
function fleetSupervisorHtmlV2(): string {
  return `<!doctype html>
...
function fleetSupervisorHtmlV3(): string {
...
function _fleetSupervisorHtml(): string {
